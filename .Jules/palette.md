...
## 2025-03-31 - LiveKit Broadcaster Camera/Mic Accessibility
**Learning:** Found that the camera and microphone toggle buttons in the `livekit-broadcaster` component used generic `size="icon"` Shadcn UI buttons without `aria-label`s, rendering them unusable by screen readers. Furthermore, their inner icons `<Mic>` and `<Video>` were read out randomly instead of hidden.
**Action:** Always add dynamic `aria-label`s reflecting state (e.g. "Turn off microphone" / "Turn on microphone") and `aria-hidden="true"` to inner decorative SVG elements in media control buttons.
