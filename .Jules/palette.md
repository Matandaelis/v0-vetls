...
## 2024-07-11 - Semantic Rating Input Form
**Learning:** Using an array of static icon components (like `RatingDisplay`) for interactive ratings creates inaccessible forms. Screen readers cannot properly understand mutually exclusive selections without semantic grouping and roles.
**Action:** When building interactive rating inputs, use individual scalable SVG icons with native `role='radiogroup'` and `role='radio'` attributes instead of static display component arrays. Use `<fieldset>` and `<legend>` for structural grouping, and ensure `aria-checked={score === star}` is used to properly reflect the mutually exclusive selection. Also, always ensure input `label`s are connected semantically with `htmlFor` and `id`.
