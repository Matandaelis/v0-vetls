...
## 2026-05-12 - Adding descriptive ARIA labels to administrative UI components
**Learning:** Found that internal tools and control panels like `StreamControlPanel` and `ProductManagement` frequently miss ARIA labels on utility icon buttons (e.g. Analytics, Edit, Delete, Volume, Share), which are heavily relied on by screen-reader users attempting to manage live-streams or inventories.
**Action:** Always verify that internal administrative panels receive the same semantic HTML and ARIA label treatment as user-facing components, taking care to embed dynamic, descriptive context (like 'Delete {product.name}') to make the actions clear in screen reader list views.
