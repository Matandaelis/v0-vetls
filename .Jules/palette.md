...
## 2025-03-05 - Interactive Rating Input Accessibility
**Learning:** Interactive rating inputs (like 5-star selectors) must be semantically structured using `<fieldset>` and `<legend>` for grouping, and `role="radiogroup"` with individual `role="radio"` buttons to reflect their mutually exclusive nature. Static display components (like `<RatingDisplay>`) used inside these buttons can cause duplicate or confusing screen reader output if they contain their own ARIA roles.
**Action:** Use native `<fieldset>` and `<legend>` for rating groups. Use individual scalable SVG icons (like `lucide-react`'s `<Star>`) with `aria-hidden="true"` inside rating buttons instead of complex display components. Ensure `aria-checked` accurately reflects the selected state.
