...
## 2025-03-05 - Native HTML Icon-Only Button Accessibility Oversight
**Learning:** While checking for `aria-label` attributes on UI library components like Shadcn `Button` with `size="icon"`, native HTML `<button>` elements containing only icons or rating stars are frequently missed and left inaccessible to screen readers.
**Action:** When auditing or implementing accessibility features, actively look for native HTML `<button>` tags, not just custom components, and ensure they have appropriate `aria-label` attributes if their purpose is visually conveyed only by icons or dynamic content like stars.
