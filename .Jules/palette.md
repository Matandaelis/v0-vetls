...
## 2025-03-31 - Missing States and Context on Stream Controls
**Learning:** Found that stream control panels (`components/host/stream-control-panel.tsx`) often utilize static, icon-only buttons for toggling stream settings like Volume/Mute, failing to provide programmatic feedback on their current state or what they do.
**Action:** When implementing or auditing stream control dashboards, always associate a local state (e.g., `isMuted`) with toggleable icon-only buttons. Combine this state with dynamic `aria-label`s and `title` attributes (e.g., `'aria-label={isMuted ? "Unmute stream" : "Mute stream"}'`) to grant clear context to both screen reader and sighted users. Don't forget to hide inner icons with `aria-hidden="true"`.
