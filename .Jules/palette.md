...
## 2024-05-19 - Notification Panel Accessibility Polish
**Learning:** Dropdowns functioning as interactive overlays often miss programmatic connections to their toggle buttons, hindering screen reader users from understanding the state (expanded/collapsed) and the relationship between the trigger and content. Additionally, native tooltips (`title`) on icon-only buttons (`aria-label`) vastly improve usability for sighted users.
**Action:** Always ensure dropdown toggles use `aria-expanded` and `aria-controls` referencing the dropdown's `id`. Supplement icon-only button `aria-label`s with `title` attributes for sighted user context, and hide decorative icons (both SVGs and emoji `<span>` elements) from screen readers using `aria-hidden="true"`.
