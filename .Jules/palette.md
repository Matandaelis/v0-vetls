...
## 2025-03-08 - Custom Interactive Components Semantics (Polls and Tip Selectors)
**Learning:** Custom UI components functioning as mutually exclusive choices (like live polls or tip amount selectors) were built using native `<button>` elements but lacked proper semantic roles. Without `role="radiogroup"` on the container and `role="radio"` with `aria-checked` on the options, screen readers cannot announce them as selectable lists, leading to confusion about their purpose and current state.
**Action:** When auditing custom interactive choice lists or segmented controls, always apply `role="radiogroup"` to the parent container and `role="radio"` with dynamic `aria-checked` attributes to the individual option buttons to ensure proper accessibility semantics.
