...
## 2026-03-26 - Dynamic Floating Icon Controls
**Learning:** Floating hotspots mapping interactive coordinates often miss labels and robust keyboard focus indicators. The combination of `aria-label` alongside `focus-visible:ring-2` and `group-focus-visible:opacity-100` ensures screen readers and keyboard users understand and perceive the control.
**Action:** When implementing coordinate-based overlay interactions, apply `group-focus-visible` to control opacity, provide dynamic ARIA labels (e.g., 'View details for product'), and explicitly hide internal decorative SVGs.
