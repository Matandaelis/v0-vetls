...
## 2025-05-18 - Dynamic ARIA Labels in Lists
**Learning:** Icon-only action buttons within a list or map (e.g., Stats, Edit, Delete for products) fail to provide adequate context for screen reader users when generic `aria-label`s like "Delete" are used. Sighted users can infer context visually from the row, but screen readers cannot.
**Action:** When adding `aria-label` and `title` attributes to icon-only buttons in repeating lists, always include the item's identifier (like `product.name`) to provide clear, actionable context (e.g., `aria-label={"Delete " + product.name}`).
