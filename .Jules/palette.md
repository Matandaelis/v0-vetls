...
## 2023-10-27 - Linking Helper Text to Inputs
**Learning:** When complex inputs (like maximum proxy bids) have accompanying explanatory text (e.g., "We'll bid for you up to this amount"), screen readers will not read the explanation when the input is focused unless explicitly linked.
**Action:** Always use `aria-describedby` on the `<input>` and apply the matching `id` to the helper text element to ensure context is announced correctly for assistive technology users.
