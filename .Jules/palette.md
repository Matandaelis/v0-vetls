...
## 2025-03-05 - Interactive Rating Semantics and Visual Redundancy
**Learning:** Interactive rating inputs (like the 5-star rating in `components/review-form.tsx`) were using a loop that rendered a display component (`<RatingDisplay>`), which itself rendered 5 stars, resulting in visually confusing output (25 stars) and broken interaction. They also lacked semantic grouping and standard accessibility attributes for mutually exclusive choices.
**Action:** When building or auditing interactive rating inputs, use individual scalable SVG icons (like `lucide-react`'s `<Star>`) instead of static display component arrays. Use `<fieldset>` and `<legend>` for structural grouping. Implement `role="radiogroup"` on the container and `role="radio"`, `aria-checked` on individual options to ensure proper semantic accessibility.
