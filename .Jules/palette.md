...
## 2025-03-24 - Stream Control Panel Accessibility Oversight
**Learning:** Found that when creating control panels with icon-only buttons (like Volume or Share in `StreamControlPanel`), developers frequently use standard UI component icon-only buttons (`<Button size="icon">`) without `aria-label`s. These buttons are inaccessible to screen readers without descriptive labels.
**Action:** Always scan for and add `aria-label` attributes to icon-only action buttons (e.g., `aria-label="Toggle Volume"`). Additionally, ensure the internal SVG/icon element has `aria-hidden="true"` to prevent screen readers from redundantly attempting to announce the graphical component.
