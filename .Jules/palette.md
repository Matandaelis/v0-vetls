...
## 2025-03-05 - Accessible Star Ratings
**Learning:** Star rating inputs created manually using arrays and state are often functionally correct for mouse users but completely inaccessible to screen readers, missing both programmatic grouping and selected state indication. Furthermore, re-using a generic display component (like `RatingDisplay`) within an interactive map causes duplicate screen reader output or nesting issues.
**Action:** When building interactive rating inputs, use individual scalable SVG icons (like `lucide-react`'s `<Star>`) with native `role='radiogroup'` and `role='radio'` attributes instead of static display component arrays. Ensure explicit size and fill styling is added directly to these raw SVGs. Use `<fieldset>` and `<legend>` for structural grouping, and ensure `aria-checked={score === star}` is used to properly reflect the mutually exclusive selection.
