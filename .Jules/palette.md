...
## 2024-06-08 - Fixed ProductManagement missing accessibility
**Learning:** Found multiple instances of icon-only action buttons (`BarChart`, `Edit`, `Trash2`) in list views (like `components/host/product-management.tsx`) lacking `aria-label`s. Also fixed a TS bug in `components/livekit/index.ts` where a type alias was incorrectly exported causing parsing errors.
**Action:** Always add dynamic `aria-label`s to icon-only buttons in map loops so screen readers can uniquely identify the item being targeted (e.g. `aria-label={"Edit " + product.name}`). Also, pair this with `title` for visual users and add `aria-hidden="true"` to inner SVG children.
