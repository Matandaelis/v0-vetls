...
## 2025-03-05 - Keyboard Focus and Hidden Elements
**Learning:** Interactive elements like carousel scroll controls that are visually hidden by default (e.g., using `opacity-0` and only shown on `group-hover`) become inaccessible to keyboard users because focusing them does not make them visible, leading to invisible tab stops.
**Action:** Whenever using `opacity-0 group-hover:opacity-100` on interactive elements, always pair it with `focus-visible:opacity-100` or `focus-within:opacity-100` to ensure keyboard accessibility.
