...
## 2024-06-03 - Form Checkbox and Radiogroup Accessibility
**Learning:** Raw star SVG elements mapping to rating values can result in duplicate and unclear announcements on screen readers if they lack structural context. Simply mapping clickable SVGs does not semantically inform users of their mutual exclusivity or selection state.
**Action:** When implementing custom rating systems or similar selectable elements, group them using `<fieldset>` and `<legend>` to provide the overarching question/purpose. Apply `role="radiogroup"` to the container and `role="radio"` with `aria-checked` to each individual option (e.g., SVG wrapper) to semantically indicate mutual exclusivity and the current state. Also link standard inputs to labels using `htmlFor` matching the input `id`.
