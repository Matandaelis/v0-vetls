...
## 2023-10-27 - [Semantic Radio Groups for Interactive Options]
**Learning:** Custom interactive elements that function as mutually exclusive choices (like polls in `live-engagement-panel.tsx`) are often implemented as standard `<div>` or list containers with generic `<button>` tags, creating an accessibility gap. Screen readers do not understand the relationship between the options.
**Action:** Always implement `role="radiogroup"` on their container and `role="radio"` with `aria-checked` on the individual options for proper semantic accessibility. Add `aria-label` to the container to associate the group with a title or question.
