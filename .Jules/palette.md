...
## 2025-03-05 - Expandable Content Semantics
**Learning:** Toggle buttons for expanding/collapsing content sections (like bid histories or FAQs) frequently lack necessary semantic bindings. Without `aria-expanded` reflecting the state and `aria-controls` pointing to the collapsible content's `id`, screen reader users have no context about the button's purpose or the state of the content it controls.
**Action:** When auditing or implementing expandable/collapsible UI elements, always ensure the trigger button includes dynamic `aria-expanded` and a static `aria-controls` attribute linking to the target container's `id`.
