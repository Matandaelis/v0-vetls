...
## 2024-04-14 - Semantic Form Label Linking
**Learning:** Found custom UI `Input` and native `textarea` elements without correctly linked `id` and `htmlFor` tags inside form layouts. It's critical to bind inputs and labels semantically. Furthermore, discovered custom interactive elements building rating inputs mapping arrays of display components without ARIA group boundaries or roles.
**Action:** Always link form labels using `htmlFor` to the corresponding input's `id`. For mutually exclusive choices like 5-star ratings, always group them with `<fieldset>` and `<legend>` and use `role="radiogroup"` with child components having `role="radio"` and `aria-checked`.
