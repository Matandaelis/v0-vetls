...
## 2024-05-18 - Semantic Form Ratings
**Learning:** Using static display component arrays (like `RatingDisplay`) for interactive, selectable rating inputs breaks semantic accessibility, causes duplicate screen reader output, and limits interactive state (like `aria-checked` per star).
**Action:** Always build interactive star ratings using native HTML semantics: a `role="radiogroup"` wrapper around individual scalable SVGs (like `lucide-react`'s `Star`) with `role="radio"`, dynamic `aria-checked`, dynamic `aria-label`s, and proper keyboard focus states (`focus-visible:ring-2`).
