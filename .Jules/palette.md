...
## 2025-03-05 - Rating Component and Form State Accessibility
**Learning:** Custom interactive elements like a 5-star rating control must be structurally grouped using `<fieldset>` and `<legend>` rather than `<div>` and `<label>` to be correctly identified as a single form unit by screen readers. Furthermore, the rating choices themselves need `role="radiogroup"` on the container and `role="radio"` with `aria-checked` on the options. Form submit buttons need `aria-busy={isSubmitting}` to communicate processing state. Lastly, when replacing arrays of components in interactive controls with raw SVG icons to fix duplicate output, explicit size/fill styling is necessary.
**Action:** When building interactive rating inputs or visually grouped form controls, use `<fieldset>` and `<legend>`. Ensure radio groups use explicit ARIA roles. Apply `aria-busy` to submit buttons during async actions.
