...
## 2024-06-11 - Dynamic ARIA labels for stateful icon buttons
**Learning:** For dynamic icon-only buttons like toggle camera, mic, pin, and like, it is important to include a dynamic `aria-label` that represents the *action* rather than generic states, e.g., using `aria-label={isPinned ? "Unpin product" : "Pin product"}` rather than just `aria-label="Pin"`.
**Action:** Always implement ternary expressions or conditionals to update the `aria-label` when the icon of an icon-only button changes to convey the upcoming action to screen reader users.
