...
## 2026-04-19 - Add ARIA Labels to Icon-Only Buttons in Product Inventory
**Learning:** Ensure that list items containing identical icon-only buttons include dynamic s specifying the target object (e.g. `aria-label={`Edit ${product.name}`}`), as simple descriptive labels become redundant and unhelpful in list contexts.
**Action:** Add dynamic context to generic 'Edit/Delete' action buttons within lists or tables.
## 2024-05-14 - Add ARIA Labels to Icon-Only Buttons in Product Inventory
**Learning:** Ensure that list items containing identical icon-only buttons include dynamic `aria-label`s specifying the target object (e.g. `aria-label={\`Edit ${product.name}\`}`), as simple descriptive labels become redundant and unhelpful in list contexts.
**Action:** Add dynamic context to generic 'Edit/Delete' action buttons within lists or tables.
