...
## 2025-05-18 - Semantic Form Grouping & Custom Ratings
**Learning:** Found that custom rating input maps in `review-form.tsx` were built with generic divs and buttons, lacking correct grouping and aria states. Additionally, the map was incorrectly nesting a custom `RatingDisplay` component which itself had a 5-item map, resulting in 25 visually hidden stars and a poor semantic tree. Finally, title and textarea fields lacked explicit `htmlFor` properties to tie to inputs, and the submit button missed `aria-busy` states for loading feedback.
**Action:** Replaced the nested component map with explicit inline SVG `Star` loops with semantic `<fieldset>` and `role="radiogroup"` structures. Ensured `htmlFor` properties were added explicitly to match IDs of custom Input components, and added `aria-busy` to the submission button state.
