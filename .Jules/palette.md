...
## 2024-05-19 - Accessible List Action Buttons
**Learning:** Icon-only action buttons in mapped lists (like product management grids) often lack context for screen readers and tooltips for sighted users, reducing discoverability.
**Action:** Pair `aria-label` with `title` (using dynamic strings like `Edit ${product.name}`) on `<Button size="icon">` and add `aria-hidden="true"` to the inner SVG icon to provide accessible, contextual tooltips.
