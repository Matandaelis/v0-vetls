...
## 2024-05-24 - Semantic Grouping and Interactive Roles for Rating Inputs
**Learning:** When building interactive rating inputs (like star ratings in review forms), using individual scalable SVG icons (like `lucide-react`'s `<Star>`) with native `role='radiogroup'` and `role='radio'` attributes instead of static display component arrays prevents duplicate screen reader output and ensures the element is semantically recognized as a set of mutually exclusive choices.
**Action:** Use `<fieldset>` and `<legend>` for structural grouping of related radio elements, and ensure `aria-checked={score === star}` (strict equality) is used to properly reflect the mutually exclusive selection pattern. Also, ensure labels use `htmlFor` matching the associated input's `id`.
