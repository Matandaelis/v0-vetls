...
## 2024-06-17 - Accessible Rating Form and Semantic Inputs
**Learning:** Replaced the custom `<RatingDisplay>` mapped array in interactive input areas with explicit raw scalable `<Star>` SVGs that include `aria-checked` states and semantic attributes. Discovered that linking title/text inputs in a form requires correct application of `htmlFor` matching the specific `id` attributes. Grouping related interactive sub-sections utilizing `<fieldset>` and `<legend>` along with radio group mappings enables full screen reader interpretability.
**Action:** When creating new components featuring star rating inputs, use native icon paths/SVG components mapping directly rather than abstracting to display components. Ensure the inputs are wrapped within semantic container structures like `<fieldset>`.
