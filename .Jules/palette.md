...
## 2026-04-27 - Added Missing ARIA Labels to Interactive Elements
**Learning:** Native `<button>` elements and Shadcn `<Button size="icon">` usage without text content frequently lack descriptive text, presenting a barrier to screen reader users. This was specifically noticed in sidebar comment list and product hotspot modal context elements.
**Action:** Always provide an explicit `aria-label` for icon-only buttons (like heart, close, send) and inputs lacking visual labels to ensure complete keyboard navigation and screen reader functionality.
