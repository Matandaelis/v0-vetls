...
## 2024-05-24 - Live Engagement Panel Poll ARIA
**Learning:** Custom poll options mimicking radio behavior must implement `role="radiogroup"` on their container and `role="radio"` on the options, using `aria-checked` to indicate the selected state.
**Action:** When building or auditing custom interactive polls or multiple-choice questions, ensure they are semantically structured as radio groups to properly convey their mutually exclusive nature to assistive technologies.
