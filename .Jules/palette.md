...
## 2026-04-10 - Accessible Rating Components with Interactive SVGs
**Learning:** When building interactive star rating inputs, looping over a distinct display component that itself loops over 5 stars causes a duplication effect (rendering 25 stars instead of 5). Furthermore, it prevents the individual buttons from being correctly grouped and identified by screen readers.
**Action:** When building interactive rating inputs (like in `components/review-form.tsx`), loop over native HTML `<button>` elements that wrap individual scalable SVG icons (like `lucide-react`'s `<Star>`), rather than static display components. Apply `role="radiogroup"` to the container and `role="radio"` and `aria-checked` to the individual buttons to ensure they function semantically as a group of radio buttons.
