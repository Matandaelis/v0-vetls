...
## 2025-02-12 - Ensure carousel scroll buttons are accessible
**Learning:** Icon-only scroll buttons for carousels often lack `aria-label` and `focus-visible` styles which hides them from screen readers and makes keyboard navigation difficult.
**Action:** Always add `aria-label` to icon-only carousel scroll buttons and make sure `focus-visible:opacity-100` or similar styles are applied so that they are visible when receiving keyboard focus.
