...
## 2024-04-08 - Pseudo-element Block Links and Dynamic Button States
**Learning:** Fixing invalid HTML where `<Button>` components are nested within Next.js `<Link>` elements requires utilizing the pseudo-element block link pattern (`relative group` on the container, `<Link className="before:absolute before:inset-0 before:z-0">` on the title, and `relative z-10` on independent interactive elements) to prevent click deadzones. Additionally, dynamic button labels (like "Add to Cart" -> "Added!") are not announced by screen readers without utilizing an `aria-live` region.
**Action:** When creating clickable cards with internal buttons, employ the pseudo-element block link pattern instead of wrapping the entire card in a `<Link>`. Use `aria-live="polite"` and `aria-atomic="true"` on dynamic text changes inside buttons to ensure screen readers announce state updates without reloading.
