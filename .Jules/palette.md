...
## 2024-06-07 - Dynamic Labels for List-Based Icon Buttons
**Learning:** When adding accessibility to icon-only buttons inside repeated lists (like a mapped product inventory), generic static labels like "Edit" lack context for screen reader users. Furthermore, sighted users also struggle to understand the target of the action without visual context or a tooltip.
**Action:** Pair dynamic `aria-label` attributes with matching `title` attributes (e.g., `aria-label={"Edit " + item.name}` and `title={"Edit " + item.name}`) to provide specific context for both screen reader announcements and native visual tooltips. Always hide the inner SVG with `aria-hidden="true"`.
