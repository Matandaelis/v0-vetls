...
## 2025-03-05 - Expand/Collapse Accessibility and Form Associations
**Learning:** Toggle buttons for expandable content (like a bid history panel) often lack `aria-expanded` and `aria-controls` attributes, making their state invisible to screen readers. Additionally, when using custom components like Shadcn `Input` within a form, labels often lack the `htmlFor` attribute linking to the input's `id`, breaking the semantic association.
**Action:** When implementing expand/collapse toggles, always add `aria-expanded`, `aria-controls`, and ensure the target container has a matching `id`. For form fields, always pair a `<label htmlFor="id">` with `<Input id="id">`.
