...
## 2024-05-21 - Accessible Interactive Rating Inputs
**Learning:** When building interactive rating inputs, using arrays of custom display components (like `RatingDisplay`) within interactive elements can lead to duplicate screen reader output or lack proper semantic meaning.
**Action:** Use raw scalable SVGs (like `lucide-react`'s `<Star>`) with explicit `aria-hidden="true"` inside native `role="radio"` buttons. Group the entire input with `<fieldset>`, `<legend>`, and `role="radiogroup"`, ensuring `aria-checked` accurately reflects the selected state.
