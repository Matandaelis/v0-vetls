...
## 2026-04-18 - Review Form Accessibility Improvements\n**Learning:** Implementing rating fields with an array of identical visual components (like stars) requires wrapping them in a `<fieldset>` with a `<legend>` and giving the container `role="radiogroup"`. The individual buttons must have `role="radio"`, a dynamically calculated `aria-checked`, and an explicit `aria-label`. Additionally, form inputs and textareas must be programmatically linked to their labels using matching `id` and `htmlFor` attributes to be accessible to screen readers.\n**Action:** Use native HTML elements where possible and apply explicit semantic ARIA roles (, , , ) for custom interactive components and form submissions.
## 2026-04-18 - Review Form Accessibility Improvements
**Learning:** Implementing rating fields with an array of identical visual components (like stars) requires wrapping them in a <fieldset> with a <legend> and giving the container role='radiogroup'. The individual buttons must have role='radio', a dynamically calculated aria-checked, and an explicit aria-label. Additionally, form inputs and textareas must be programmatically linked to their labels using matching id and htmlFor attributes to be accessible to screen readers.
**Action:** Use native HTML elements where possible and apply explicit semantic ARIA roles ('radiogroup', 'radio', 'aria-checked', 'aria-busy') for custom interactive components and form submissions.
