...
## 2025-03-08 - Accessible Rating Forms and Loading States
**Learning:** Found that custom rating input interfaces (like 5-star selectors) are often built as a generic `div` of buttons, lacking semantic form grouping and mutually-exclusive state information for screen readers. Furthermore, text inputs frequently miss programmatic associations to their labels. Lastly, while submit buttons usually become `disabled` during async operations, they don't explicitly inform screen readers that the form is processing.
**Action:** Always wrap custom rating inputs in `<fieldset>`/`<legend>` and apply `role="radiogroup"` to the container with `role="radio"` and `aria-checked` on each option. Use explicit `htmlFor` and `id` linking for all `<label>`/`<input>` pairs. Ensure async submit buttons receive `aria-busy={isSubmitting}` to explicitly convey the loading state.
