...
## 2024-05-19 - Verified UI Element Existence Before Planning
**Learning:** When attempting to add accessibility features to a component based on an assumption of what is typically present (e.g., an Advanced Settings panel), it is critical to verify the actual existence and structure of those elements in the code first. Plans that assume the presence of unverified code structures (like `showAdvanced` or an `advanced-controls-panel` ID) violate the groundedness rule and will fail review.
**Action:** Always use exploration tools (`grep`, `cat`) to examine the target file and confirm the exact names, state variables, and HTML structures present before writing an execution plan to modify them.
