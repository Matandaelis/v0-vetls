...
## 2025-03-05 - Accessible Rating Forms
**Learning:** Rating input widgets built with mapped buttons and decorative SVGs frequently lack proper form semantics, leaving them inaccessible to screen readers. They require `fieldset`/`legend` grouping and `radiogroup`/`radio` roles to function similarly to native radio inputs. Text inputs also frequently miss standard `id` and `htmlFor` label linkage.
**Action:** When building interactive rating inputs, use individual scalable SVG icons with native `role='radiogroup'` and `role='radio'` attributes instead of static display component arrays. Use `<fieldset>` and `<legend>` for structural grouping, and ensure `aria-checked` reflects the mutually exclusive selection. Semantically link all related labels and inputs with `htmlFor` and `id`.
