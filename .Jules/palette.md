...
## 2024-04-16 - Pseudo-Element Block Link Pattern for Cards
**Learning:** Next.js `<Link>` components wrapped around interactive UI components (like `<Card>`s containing `<Button>`s) generate invalid HTML structures (`<a>` wrapping `<button>`) and cause hydration errors or click intercept issues. Wrapping the whole card in an anchor causes nested interactive element accessibility issues.
**Action:** Use the pseudo-element block link pattern instead: apply `relative group` to the container `<Card>`, place the `<Link>` inside a descriptive heading with `className="before:absolute before:inset-0 before:z-0"`, and add `relative z-10` to any independent interactive elements (like an "Add to Cart" button) within the card to preserve their focusability and clickability without triggering the main link.
