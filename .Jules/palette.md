...
## 2024-06-03 - Accessible Form Validation and Rating Components
**Learning:** Interactive rating components built from static component arrays often trigger duplicate screen reader output and lack proper mutual exclusivity context. Additionally, inputs lacking associated labels (`htmlFor`) make form navigation difficult for assistive technologies. Adding `aria-busy` to submit buttons ensures users are informed of ongoing asynchronous submissions.
**Action:** Always build rating inputs using individual SVG/icon components wrapped in a `<fieldset>`, employing `role="radiogroup"` and `role="radio"` with explicit `aria-checked` states. Ensure all form fields are semantically linked using `htmlFor` matching the input's `id`.
