...
## 2025-03-05 - Live Engagement Panel Accessibility Requirements
**Learning:** Custom poll interfaces often lack proper grouping and mutually exclusive state announcements for screen readers. In `LiveEngagementPanel`, the poll options were simple buttons. Additionally, icon-only action buttons like emoji reactions and context-specific inputs like custom tip amounts lacked explicit ARIA labels.
**Action:** When auditing custom radiogroup-like components (such as polls or single-choice surveys), ensure the container has `role="radiogroup"` and a descriptive `aria-label`. Individual options must have `role="radio"` and dynamically manage `aria-checked`. Ensure all icon-only buttons (like emoji reactions) and contextual inputs (like tip amounts) have descriptive `aria-label`s.
