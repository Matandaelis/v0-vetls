...
## 2024-05-24 - Dynamic State ARIA Labels
**Learning:** For interactive UI elements functioning as toggles that only display icons (like Camera or Microphone on/off buttons), generic labels are insufficient. Screen readers need to know the *action* that will occur when pressed, derived dynamically from component state, rather than just the generic name. Furthermore, sighted users also rely on this context.
**Action:** When working with toggle-style icon-only buttons, always apply dynamic `aria-label` attributes reflecting the action (e.g. `isMicEnabled ? "Turn off microphone" : "Turn on microphone"`), paired with an identical `title` attribute to provide tooltips for sighted users. Additionally, add `aria-hidden="true"` to the internal decorative SVG/icon components.
