...
## 2024-09-11 - Dynamic ARIA labels for comment interactions
**Learning:** Found an existing pattern for "Like comment" buttons in `components/show-sidebar.tsx` that includes a dynamic `aria-label` with the user name and the current number of likes. It's a great pattern but wasn't uniformly applied—the similar button in `components/show-chat.tsx` lacked dynamic context, focus styles, and hid the decorative icon.
**Action:** Always replicate dynamic `aria-label` and `title` patterns consistently across all implementations of chat/comment interactions (e.g. `aria-label={\`Like comment from \${comment.userName}\${comment.likes > 0 ? \`, \${comment.likes} likes\` : ''}\`}`). And always add keyboard focus styles to native HTML buttons used in custom components.
