...
**Learning:** Elements that are hidden by default and revealed on hover (e.g., using `opacity-0 group-hover:opacity-100`) are invisible to keyboard users who navigate via <kbd>Tab</kbd>, even when they receive focus. This creates a confusing "invisible focus" state.
**Action:** Always pair `opacity-0 group-hover:opacity-100` with a focus-visibility class like `focus-visible:opacity-100` or `focus:opacity-100` (along with `transition-opacity`) to ensure interactive elements become visible when accessed via keyboard navigation.
