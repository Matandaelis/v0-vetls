...
## 2024-05-18 - [Fix nested interactive elements using pseudo-element block link pattern]
**Learning:** Wrapping a whole card with a `<Link>` that contains interactive elements like `<Button>` leads to invalid HTML and creates nested interactive issues for screen readers. Next.js does not allow a `<button>` inside an `<a>` tag implicitly.
**Action:** Use the "pseudo-element block link" pattern instead. Apply `relative group` to the container, place the `<Link>` inside a heading with `before:absolute before:inset-0 before:z-0` so it stretches over the whole card, and add `relative z-10` to independent interactive elements to prevent click dead zones without invalid nesting.
