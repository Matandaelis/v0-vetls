...
## 2025-03-24 - Host Stream Control Accessibility Pattern
**Learning:** Icon-only control buttons in host stream management interfaces (like the StreamControlPanel) often lack accessible names for screen readers, leading to confusing or silent announcements for essential actions (e.g., toggling audio, screen sharing, etc.).
**Action:** Always add descriptive `aria-label` attributes to `Button` components that only render icons, add `title` attributes to serve as visual tooltips for sighted users, and use `aria-hidden="true"` on the icon itself to prevent screen readers from reading meaningless SVGs.
