...
## 2024-03-31 - Semantic Feedback and Keyboard Support in Review Toggles
**Learning:** Found that when creating interactive toggle buttons (like "Helpful" in `components/review-card.tsx`), developers frequently omit the `aria-pressed` attribute which is essential for screen reader users to understand the current state of a toggle. Also, using a dynamic `aria-label` that includes the visible text (like the vote count) prevents screen readers from completely ignoring the visible data when reading the label. Furthermore, using a `disabled` state to prevent double-voting breaks standard UX patterns; users expect to be able to toggle (undo) their vote.
**Action:** When auditing or implementing interactive toggle components, always include `aria-pressed={state}`, use dynamic `aria-label`s that incorporate any visible text within the button, ensure the decorative icons have `aria-hidden="true"`, and implement full toggle functionality (vote/unvote) rather than relying on a disabled state.
