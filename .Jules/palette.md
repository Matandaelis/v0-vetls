...
## 2025-03-05 - Dynamic Aria Labels on Video Conference Buttons
**Learning:** In video conference components, dynamic toggle states (e.g., mute/unmute, camera on/off, pin/unpin) often lack descriptive  attributes reflecting their current state, relying purely on visual icon changes (like `<Mic>` vs `<MicOff>`).
**Action:** When implementing or auditing toggle buttons, ensure the `aria-label` dynamically reflects the action the user is about to take (e.g., `aria-label={isMicEnabled ? "Mute microphone" : "Unmute microphone"}`) rather than just the generic function.

## 2025-03-05 - Dynamic Aria Labels on Video Conference Buttons
**Learning:** In video conference components, dynamic toggle states (e.g., mute/unmute, camera on/off, pin/unpin) often lack descriptive `aria-label` attributes reflecting their current state, relying purely on visual icon changes (like `<Mic>` vs `<MicOff>`).
**Action:** When implementing or auditing toggle buttons, ensure the `aria-label` dynamically reflects the action the user is about to take (e.g., `aria-label={isMicEnabled ? "Mute microphone" : "Unmute microphone"}`) rather than just the generic function.
