...
## 2026-05-07 - Rating Inputs with Native Radio Group\n**Learning:** When creating custom star rating inputs, avoid rendering distinct display component arrays within buttons. Instead, map single SVG elements directly and wrap them in a `<fieldset>` with `role="radiogroup"`. Each button requires `role="radio"`, strict equality for `aria-checked`, and an individual `aria-label`.\n**Action:** Apply this radiogroup structure for any custom rating or mutually exclusive visual selection components.
