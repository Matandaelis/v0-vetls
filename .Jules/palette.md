...
## 2024-09-04 - Review Helpful Buttons
**Learning:** "Helpful" feedback buttons need toggle states and clear descriptions of current counts to assist screen readers, and visual feedback for the selected state.
**Action:** Apply `aria-pressed`, dynamic `aria-label` including current counts, and `fill-current` visual state changes to rating/feedback mechanisms.
