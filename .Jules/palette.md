...
## 2025-03-05 - Redundant List Icon Button Labels
**Learning:** In list views (like `product-management.tsx` or `stream-control-panel.tsx`), multiple identical icon-only buttons appear next to each other. Relying on a generic label like "Edit" is confusing. Furthermore, screen readers will often redundantly announce the SVG content if not hidden.
**Action:** When making list-based icon buttons accessible, always include item-specific context in the `aria-label` and `title` (e.g., `aria-label={\`Edit \${product.name}\`}`), and ensure the inner icon has `aria-hidden="true"`.
