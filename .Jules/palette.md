...
## 2025-03-05 - Icon-Only Action Buttons in Video Overlays
**Learning:** In highly visual components like `ClipCard` (`components/clip-card.tsx`), interactive overlay buttons (Like, Chat, Share) are frequently designed as small, floating, icon-only circles to maximize video visibility. These are often missed in accessibility audits because they visually "blend in" with the media. Without `aria-label` attributes, screen reader users only hear unhelpful announcements or nothing at all. Additionally, the internal `lucide-react` SVGs must be explicitly hidden with `aria-hidden="true"` to prevent redundant reading if the button itself is labeled.
**Action:** Always check absolute positioned or overlay buttons over media elements for missing `aria-label` and `aria-hidden` attributes.
