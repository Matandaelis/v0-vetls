...
## 2024-06-05 - Enhance review form accessibility
**Learning:** Rating inputs in review forms are often implemented as a visual display of stars with click handlers, but lack semantic structure. It's critical to treat them as a mutually exclusive choice (like a radio group) using `role="radiogroup"` and `role="radio"` with `aria-checked` attributes. Additionally, using standard SVGs over complex display components for these inputs prevents duplicate screen reader announcements.
**Action:** When building interactive rating inputs, use individual scalable SVG icons with native `role='radiogroup'` and `role='radio'` attributes instead of static display component arrays to prevent duplicate screen reader output. Use `<fieldset>` and `<legend>` for structural grouping, and ensure `aria-checked` properly reflects the mutually exclusive selection. Form submit buttons should utilize `aria-busy={isSubmitting}`.
