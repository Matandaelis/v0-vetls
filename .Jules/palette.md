...
## 2025-05-16 - Accessible Rating Component Pattern
**Learning:** When building interactive rating inputs (like star ratings), mapping over an array to render individual buttons that each internally render a full visual component (e.g., `<RatingDisplay>`) results in a massive multiplication of DOM nodes (e.g., 25 SVG stars for a 5-star rating) and completely breaks accessibility semantics.
**Action:** Always build interactive rating components using a native `<fieldset>` and `<legend>` for structural grouping. Inside, use individual scalable SVG icons (like `lucide-react`'s `<Star>`) mapped over the options. The container must have `role="radiogroup"`, and each button must have `role="radio"`, `aria-checked={score === star}`, and a distinct `aria-label` to provide proper semantic meaning and feedback to screen readers.
