...
## 2026-04-02 - Interactive Rating Input Semantics
**Learning:** In interactive rating inputs (like 5-star selectors), relying on visual state alone is insufficient. Rendering a `RatingDisplay` component inside each button led to rendering 25 total stars (5 stars inside each of the 5 buttons), causing visual bugs and severe screen reader redundancy.
**Action:** When building interactive rating inputs, use individual scalable SVG icons with native `role="radiogroup"` and `role="radio"` attributes instead of static display component arrays to prevent duplicate screen reader output and ensure proper semantic functionality.
