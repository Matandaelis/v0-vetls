...
## 2026-05-28 - Add missing aria-labels to icon-only buttons
**Learning:** Icon-only action buttons (e.g., using `Button size="icon"` with a Lucide React icon inside) lack semantic meaning for screen readers. In this project's recurring patterns, we pair `aria-label` with `title` for visual tooltips, and add `aria-hidden="true"` to the inner SVG component.
**Action:** Always ensure icon-only buttons receive `aria-label` to describe the action, `title` to provide sighted users with context (since there's no visible text), and add `aria-hidden="true"` to the decorative icon component itself.
