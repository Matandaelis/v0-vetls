...
## 2026-04-02 - Block Link and Aria Live
**Learning:** Next.js `<Link>` wrappers around entire interactive cards result in invalid HTML (`<button>` nested in `<a>`) and poor screen reader experiences.
**Action:** Use the 'pseudo-element block link' pattern: set the card to `relative group`, place the `<Link>` inside a heading with `before:absolute before:inset-0 before:z-0`, and ensure independent interactive elements inside the card use `relative z-10` to stay clickable. Add `aria-live="polite"` to buttons with dynamic text changes (like 'Add to Cart' to 'Added!') to announce state changes properly.
