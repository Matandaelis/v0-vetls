...
## 2024-05-18 - Improve Carousel Accessibility Focus State
**Learning:** Icon-only buttons in interactive carousels that use `opacity-0` with `group-hover:opacity-100` for visual cleanliness are completely invisible to keyboard navigators (who tab to the element) unless explicitly styled.
**Action:** Always include `focus-visible:opacity-100` alongside hover opacity transitions to ensure the element becomes visible when it receives keyboard focus. Also ensure proper `aria-label`s and `aria-hidden="true"` on inner SVGs are added.
