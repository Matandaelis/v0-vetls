...
## 2025-03-05 - Dynamic ARIA Labels in Lists
**Learning:** Icon-only buttons used inside mapping/list contexts (like an inventory list) need dynamic `aria-label`s. Using static `aria-label`s (like "Delete") results in duplicate screen reader announcements that lack context.
**Action:** Always interpolate item-specific data (e.g., ``aria-label={`Delete ${product.name}`}``) for icon-only action buttons rendered inside a list or grid context.
