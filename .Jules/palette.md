...
## 2024-06-22 - Add ARIA Labels and Keyboard Access to Carousel Controls
**Learning:** Carousel scroll controls often rely on generic, unlabelled icons (e.g. `<ChevronLeft />`, `<ChevronRight />`) and are sometimes visually hidden until hover, making them completely inaccessible to keyboard and screen reader users.
**Action:** Always add descriptive `aria-label` and `title` attributes to carousel navigation buttons. Crucially, when buttons use opacity classes for hover effects (e.g., `opacity-0 group-hover:opacity-100`), ensure `focus:opacity-100` and `focus-visible:opacity-100` are also added so they become visible during keyboard navigation.
