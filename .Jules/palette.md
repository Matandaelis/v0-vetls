...
## 2024-06-12 - Interactive Rating Inputs Accessibility
**Learning:** Building interactive rating inputs correctly requires individual native focusable elements (like buttons with role='radio') grouped within a 'radiogroup' and a parent 'fieldset' rather than static arrays of display components to ensure screen readers announce options distinctly and mutually exclusive states correctly.
**Action:** Always replace mapping over static component arrays (e.g. `RatingDisplay`) inside buttons with scalable inline SVGs, adding `role="radio"`, `aria-checked`, and `aria-label` attributes to each interactive option.
