...
## 2024-05-27 - Radio Group Accessibility for Custom Polls
**Learning:** Custom interactive elements functioning as mutually exclusive choices (like the poll options in `LiveEngagementPanel`) were built using standard `<button>` tags without semantic grouping. This causes screen readers to announce them as independent buttons rather than related options, confusing users about the interaction model.
**Action:** Always wrap custom single-choice lists in a container with `role="radiogroup"` (and an appropriate `aria-label`), and apply `role="radio"` and `aria-checked={isSelected}` to each interactive option.
