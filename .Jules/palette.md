...
**Learning:** Found that host control panels often use generic icon-only buttons for actions like Mute or Share, without native tooltips (`title`) or screen reader context (`aria-label`). Sighted users lack context without tooltips, and screen readers read redundant SVGs.
**Action:** When auditing host controls or similar tools, pair `aria-label` with `title` for tooltip support and add `aria-hidden="true"` to inner SVG/Lucide icons. Implement toggle state for labels where applicable.
