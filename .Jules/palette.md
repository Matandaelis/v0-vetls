...
## 2025-03-05 - Dynamic Aria Labels for List Action Buttons
**Learning:** Icon-only action buttons within repeating lists (like the Product Management inventory grid) require dynamic `aria-label` attributes to distinguish actions. A generic "Delete" label is confusing when there are multiple delete buttons. Additionally, `aria-hidden="true"` must be added to the nested icons to prevent screen readers from redundantly announcing the icon along with the button's label.
**Action:** When adding icon-only buttons in map/list structures, always include the item's name or identifier in the `aria-label` (e.g., `aria-label={"Delete " + item.name}`). Apply `aria-hidden="true"` to inner decorative SVG elements.
