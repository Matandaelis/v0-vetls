...
## 2025-03-24 - Dynamic ARIA and Popover State on Absolute Positioned Map Markers
**Learning:** Found that absolute positioned native HTML `<button>` elements acting as image hotspots/markers within `.map` loops often lack keyboard focus states (`focus-visible`) and semantic popover links (`aria-expanded`, `aria-controls`). Because they are context-free visual dots, they critically need dynamic `aria-label`s, which often requires looking up associated data inside the loop (e.g. finding a product by ID).
**Action:** When implementing clickable map markers or hotspots, ensure they have custom keyboard focus outlines, use `aria-expanded` and `aria-controls` to link to their popover/card content, and look up contextual data inside the map loop to populate an accurate `aria-label`.
